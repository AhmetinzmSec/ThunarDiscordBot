...
async function getHelpMenu({client, guild}) {
    ...
    let components = [];
    components.push(
        new ButtonBuilder().setCustomId("previousBtn").setEmoji("<:sol:1062406654852874240>").setStyle(ButtonStyle.Success).setDisabled(true),
        new ButtonBuilder().setCustomId("dis").setLabel("🔍").setStyle(ButtonStyle.Secondary).setDisabled(true),
        new ButtonBuilder().setCustomId("nextBtn").setEmoji("<:sag:1062406651774242846>").setStyle(ButtonStyle.Success).setDisabled(true)
    );
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(
            "**About Thunar:**\n" +
            `Hello. I am ${guild.members.me.displayName}!. I make every effort to serve you in the name of security. I am constantly armed with security patches and huge monthly updates. I try to improve my service with brand new systems and updated systems every month.\n\n ✥────────────────────────────✥\n\n` +
            "My infrastructure has brought a lot of new features with the giant update that takes place every year and in 2023. It lost a few features at the same time, albeit temporarily. My developers are still scrambling to add these systems. With the new infrastructure update, we got rid of most entertainment commands and moved to the 'Do one job, do it without mistakes' philosophy. Security patches and updates will come much more frequently with the 2023 update.\n\n" +
            `**Invite Server:** [Here](${client.getInvite()})\n` +
...
const waiter = (msg, userId, prefix) => {
    ...
    collector.on("end", () => {
        ...
        return msg.editable && msg.edit({content: "❗**The menu is now de-active**", components: []});
    });
};
